...
void RpvSoftsusy::outputGutScaleNonZeroInputs(ostream & out) {
  ...
  for (i=1; i<=3; i++)
    for (j=1; j<=3; j++) 
      for (k=1; k<=3; k++){
	if (fabs(displayLambda(LE)(k, i, j)) > 6.66e-66) {
	    if (!anyCoups) { 
	      anyCoups = true;
	      out << "Block RVLAMLLEIN                  # GUT scale LLE couplings \n";
	    }
	    out << "  " << i << " " << j << " " << k 
		<< "  "; printRow(out, displayLambda(LE)(k, i, j));
	    out << "   # lambda_{" << i << j << k << "}\n"; 
	  }     
      }
  anyCoups = false;
  ...
  for (i=1; i<=3; i++)
    for (j=1; j<=3; j++) 
      for (k=1; k<=3; k++){
	if (fabs(displayLambda(LD)(k, i, j)) > 6.66e-66) {	
	    if (!anyCoups) { 
	      anyCoups = true;
	      out << "Block RVLAMLQDIN                  # GUT scale LLE couplings \n";
	    } 
	    out << "  " << i << " " << j << " " << k 
		<< "  "; printRow(out, displayLambda(LD)(k, i, j));
	    out << "   # lambda'_{" << i << j << k << "}\n";      
	  }
      }
  anyCoups = false;
  ...
  for (i=1; i<=3; i++)
    for (j=1; j<=3; j++) 
      for (k=1; k<=3; k++){
	if (fabs(displayLambda(LU)(i, j, k)) > 6.66e-66) {
	    if (!anyCoups) { 
	      anyCoups = true;
	      out << "Block RVLAMUDDIN                 # GUT scale LQD couplings \n";
	    } 
	    out << "  " << i << " " << j << " " << k 
		<< "  "; printRow(out, displayLambda(LU)(i, j, k));
	    out << "   # lambda''_{" << i << j << k << "}\n";      
	  }
      }
}
...
void RpvSoftsusy::lesHouchesAccordOutput(ostream & out, const char model[], 
