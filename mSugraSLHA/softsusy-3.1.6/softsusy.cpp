/** \file softsusy.cpp Project: SOFTSUSY Author: Ben Allanach Manual:
   hep-ph/0104145, Comp. Phys. Comm. 143 (2002) 305 Webpage:
...
void MssmSoftsusy::calcTadpole1Ms1loop(double mt, double sinthDRbar) { /// checked
  ...
  //  physpars.t1OV1Ms = physpars.t1OV1Ms1loop; DEBUG taken out think it's wrong
}
...
void MssmSoftsusy::calcTadpole2Ms1loop(double mt, double sinthDRbar) {/// CHECKED
  ...
  //  physpars.t2OV2Ms = physpars.t2OV2Ms1loop; DEBUG changed by Ben 28/4/10
}
...
void MssmSoftsusy::gluino(int accuracy) {
  ...
  /// Matchev says BPMZ may be wrong!
...
void MssmSoftsusy::physical(int accuracy) {
  ...
  if (higgsTachyon) flagTachyon(true);
...
(void (*boundaryCondition)(MssmSoftsusy &, const DoubleVector &),
 ...
 oneset, bool gaugeUnification, bool ewsbBCscale) {
  ...
  try {
    ...
    cerr << "SOFTSUSY problem: " << a << " pars=" << pars << " tanb=" << tanb 
	 << " oneset=" << oneset << endl;
    ...
    cerr << "SOFTSUSY problem: " << endl;
    cerr << "pars=" << pars << " tanb=" << tanb
	 << " oneset=" << oneset << endl;
    flagProblemThrown(true);
  }
...
void MssmSoftsusyAltEwsb::rewsb(int sgnMu, double mt, double muOld) {
  ...
  double newMh1sq, newMh2sq;
    ...
    setM3Squared(m3sqnew);
    /* DEBUG checking mA calculation
       cout << " Calculated mAsqRun=" << m3sqnew * (tanb + 1.0 / tanb) << " or " << 2.0 * sqr(displaySusyMu()) +  newMh1sq + newMh2sq - displayTadpole1Ms() -
       displayTadpole2Ms() << " dMA=" << dMA << " b_A=" << -sqr(sin(beta)) * displayPhys().t1OV1Ms1loop - sqr(cos(beta)) * displayPhys().t2OV2Ms1loop << " piaa=" << piaa;
           
       double masqpole = 1.0 / cos(2.0 * beta) * 
       (newMh2sq - newMh1sq + displayTadpole1Ms() - displayTadpole2Ms()) - 
      sqr(mzRun) - piaa + dMA + sqr(sin(beta)) * displayPhys().t1OV1Ms1loop + 
      sqr(cos(beta)) * displayPhys().t2OV2Ms1loop;
      cout << " mAsqpole=" << masqpole;
    */
    if ((displayMh1Squared() + 2.0 * sqr(displaySusyMu()) +
       displayMh2Squared() - 2.0 * fabs(displayM3Squared())) < 0.0 )
    flagHiggsufb(true);
