...
//The classes and their member functions
...
class ABCAlphaBeta
{
   private:
      ...
      double etaMinFit_;
      double etaMaxFit_;
      double etaMinEtaFit_;
      double etaMaxEtaFit_;
      bool isBarrel_;
   ...
   public:
      ...
      void computeETrueAverage();
      void computeETrueRMS();
      void computeA(double a);
      void computeB();
      void computeC();
      bool computeBC();
...
bool ABCAlphaBeta::addEntry(double ETrue, double ecalEnergy, double hcalEnergy, double eta)
{
   ...
   etas_.push_back(eta);
   sigmaEcalHcal_.push_back(sigmaEcalHcal); //Not sure about this ask Patrick.
...
//Unsure about this, ask Rick.
} 
void ABCAlphaBeta::computeA(double a) 
{
   a_ = a; //May want to change this iterative calculation.
}
...
class Calibration
{
   ...
   public:
      ...
      bool fitAsToFunction(TF1 *functionA);
      bool fitAsToFunction();
      bool fitBsToFunction(TF1 *functionB);
      bool fitBsToFunction();
      bool fitCsToFunction(TF1 *functionC);
      ...
      void drawCoeffGraph(string graph);
      void drawSigmaGraph(string graph);
...
{
   graphC_->Fit(functionC_->GetName(), "Q", "", 0, ETrueMax_);
   cout<<functionC_->GetExpFormula("P")<<endl;
   return true;
}
...
void Calibration::drawCoeffGraph(string graph)
{
   ...
   TH2F* histo = new TH2F("histo", "", 100, 0, ETrueMax_, 100,  -2.0, 2.0); 
...
void drawGausFit(TH2F* inHisto)
{
   ...
   for(unsigned bin = 2; bin < 1000; )
   {
      ...
      ETrueBin.back()->Fit("gaus", "Q", "",
                           gaus->GetParameter(1) - 2*gaus->GetParameter(2), 1.0);     
      gaus = ETrueBin.back()->GetFunction("gaus");
      ETrueBin.back()->Fit("gaus", "Q", "",
                           gaus->GetParameter(1) - 2*gaus->GetParameter(2), 1.0);
      gaus = ETrueBin.back()->GetFunction("gaus");
   ...
   response = TGraph(ETrue.size(), &ETrue[0], &gausMean[0]);
...
TF1* functionBarrelEcalHcalA;
TF1* functionBarrelEcalHcalB;
...
vector<double> ETrueEnergies;
vector<double> ecalEnergies;
vector<double> hcalEnergies;
vector<double> etas;
vector<double> phis;

vector<ABCAlphaBeta*> barrelWithEcalHcal;
vector<ABCAlphaBeta*> barrelWithEcal;
vector<ABCAlphaBeta*> barrelWithHcal;
...
void calibChris() 
{
   ...
   for(unsigned bin = 2; bin < barrelWithEcalHcal.size() - 1; ++bin)
   {
   ...
   functionBarrelHcalC->FixParameter(4,1.2);
   functionBarrelHcalAlpha->SetParameters(0.02, -0.1, 200);
   functionBarrelHcalBeta->SetParameters(-0.02, 0.4, 200, 0.0, 0.0);
   functionBarrelHcalBeta->FixParameter(3, 0.0);
   functionBarrelHcalBeta->FixParameter(4, 0.0);
   ...
   functionEndcapHcalBeta->SetParameters(-0.02, 0.4, 200, 0.0, 0.0);
   functionEndcapHcalBeta->FixParameter(3, 0.0);
   functionEndcapHcalBeta->FixParameter(4, 0.0);
   ...
   //drawGausFit(uncorrectedBarrelEcalHcal);
   //drawGausFit(correctedBarrelEcalHcal);
   //drawGausFit(correctedBarrelEtaEcalHcal);

   //drawGausFit(uncorrectedBarrelHcal);
   //drawGausFit(correctedBarrelHcal);
   //drawGausFit(correctedBarrelEtaHcal);

   //drawGausFit(uncorrectedEndcapEcalHcal);
   //drawGausFit(correctedEndcapEcalHcal);
   //drawGausFit(correctedEndcapEtaEcalHcal);

   //drawGausFit(uncorrectedEndcapHcal);
   //drawGausFit(correctedEndcapHcal);
   //drawGausFit(correctedEndcapEtaHcal);
